...
(function(){
    ...
    Game_BattlerBase.prototype.xparam = function(xparamId) {
        if ($gameSystem.isSRPGMode() && tagId >= 0 && tagId <= 9) { // Solo aplicar cambios si es un tag válido
            var isActor = this.isActor();
            var terrainMod = 1; 
        
            if (isActor) { // Si es un aliado
                if (tagId >= 1 && tagId <= 3) { 
                    terrainMod = 2; // Doble influencia en aliados
                } else if (tagId >= 4 && tagId <= 6) { 
                    terrainMod = 3; // Triple influencia en aliados
                } else if (tagId >= 7 && tagId <= 9) { 
                    terrainMod = 4; // Cuádruple influencia en aliados
                }
            } else { // Si es un enemigo
                if (tagId >= 1 && tagId <= 3) { 
                    terrainMod = 3; // Triple influencia en enemigos
                } else if (tagId >= 4 && tagId <= 6) { 
                    terrainMod = 4; // Cuádruple influencia en enemigos
                } else if (tagId >= 7 && tagId <= 9) { 
                    terrainMod = 5; // Quintuple influencia en enemigos
                }
            }
        
            if (terrainMod !== 1) { // Solo aplicar modificación si hay cambios en el modificador
                value *= terrainMod; // Multiplicar por el modificador
            }
        }
